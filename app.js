//configuración personal de Firebase
	...
	//Leer documentos sin check-in
		...
		db.collection("rsv").where('registered', '==', 0).onSnapshot((querySnapshot) => {
			tabla1.innerHTML = '';
			querySnapshot.forEach((doc) => {
				...
				<td><button class="btn btn-warning" onclick="editar('${doc.id}','${doc.data().rsv}','${doc.data().checkin}','${doc.data().checkout}','${doc.data().name}','${doc.data().lastname}','${doc.data().room}','${doc.data().adults}','${doc.data().childs}','${doc.data().email}','${doc.data().phone})">Editar</button></td>
				</tr>
				`
			});
		});
...
//Leer documentos con registro realizado
		...
		db.collection("rsv").where('registered', '==',true).onSnapshot((querySnapshot) => {
			tabla2.innerHTML = '';
			querySnapshot.forEach((doc) => {
				console.log(`${doc.id} => ${doc.data().name}`);
				tabla2.innerHTML += ` 
	...
	//borrar documentos
		function eliminar(id){
			db.collection("rsv").doc(id).delete().then(function() {
...
//editar documentos
	function editar(id,reservacion,checkin,checkout,nombre,apellido,room,adults,childs,correo,telefono){
	...
	console.log(reservacion,checkin,checkout,nombre,apellido,room,adults,childs,correo,telefono);
